...
class Sparse:
	def __init__(
		...
		self.ndim = len(shape)
		self.indices = np.asarray(indices) if copy is False else np.array(indices, copy=copy)
		self.values = np.asarray(values) if copy is False else np.array(values, copy=copy)
		self.indptr = None if indptr is None else (np.asarray(indptr) if copy is False else np.array(indptr, copy=copy))
		self.shape = np.asarray(shape) if copy is False else np.array(shape, copy=copy)
