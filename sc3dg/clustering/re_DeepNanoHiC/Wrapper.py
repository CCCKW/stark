...
class DeepNanoHiC():
	...
	def prep_model(self):
		...
		self.label_info = pickle.load(open(os.path.join(self.temp_dir, "label_info.pickle"), "rb"))
		...
		self.higashi_model = Hyper_SAGNN(
			...
			chrom_num=len(self.chrom_list),
			JK='last',
			num_experts=3,
			k=1, 
			coef=1).to(device, non_blocking=True)
	...
	def train_epoch(self, training_data_generator, optimizer_list, train_pool, train_p_list):
		...
		for p in as_completed(train_p_list):
			...
			for j in range(self.collect_num):
				...
				if self.use_recon:
					...
					ratio = 0.0
					ratio1 = 0.0
				load_balance_loss1 = model.extra_proba.load_balance_loss
				load_balance_loss2 = model.extra_proba2.load_balance_loss
				load_balance_loss3 = model.extra_proba3.load_balance_loss
				train_loss = self.alpha * loss_bce + ratio1 * loss_mse + self.contractive_loss_weight * contractive_loss + load_balance_loss1 + load_balance_loss2 + load_balance_loss3
				for opt in optimizer_list:
					opt.zero_grad(set_to_none=True)
				...
				moe_loss_1 += load_balance_loss1.item()
				moe_loss_2 += load_balance_loss2.item()
				moe_loss_3 += load_balance_loss3.item()
		...
		return bce_total_loss / final_batch_num, mse_total_loss / final_batch_num, accuracy(
			y.view(-1), pred.view(-1)), auc1, auc2, str1, str2, train_pool, train_p_list,moe_loss_1/final_batch_num,moe_loss_2/final_batch_num,moe_loss_3/final_batch_num,
	...
	def train(self, training_data_generator, validation_data_generator, optimizer, epochs, load_first,
		...
		for epoch_i in range(epochs):
			...
			bce_loss, mse_loss, train_accu, auc1, auc2, str1, str2, train_pool, train_p_list, moe1, moe2, moe3= self.train_epoch(
	...
	def train_for_imputation_with_nbr(self):
		...
		np.save(os.path.join(self.temp_dir, "weighted_info.npy"), np.array([cell_neighbor_list, weight_dict],dtype=object),
		        allow_pickle=True)
