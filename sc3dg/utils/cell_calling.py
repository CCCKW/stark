...
from sc3dg.utils.sgt import sgt_proportions,SimpleGoodTuringError
...
#     empty_bcs = bc_order[::-1][int(0.01 * len( bc_order)):] # bottom 5% is ambient
#     empty_bcs.sort()
...
def find_nonambient_barcodes(matrix, orig_cell_bcs,
    ...
    median_initial_umis = np.median(list(set(umis_per_bc[orig_cells])))
 
    min_umis = int(max(min_umis_nonambient, round(np.ceil(median_initial_umis * min_umi_frac_of_median))))
    print('Median UMIs of initial cell calls: {}'.format(median_initial_umis))
    print('Min UMIs: {}'.format(min_umis))
    print(sum(umis_per_bc<min_umis))

    # Take what we expect to be the barcodes associated w/ empty partitions.
    
    empty_bcs = bc_order[::-1][int(0.05 * len( bc_order)):] # bottom 5% is ambient
    empty_bcs.sort()
