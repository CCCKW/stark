...
def main():
    matrix = cooler.Cooler(sys.argv[1] + '::resolutions/1000000') # .cool filehandle
    if not cooltools.lib.is_cooler_balanced(matrix):
        cooler.balance_cooler(matrix, store=True)
...
def calculate_giniqc(matrix):
    
    normalized, total_reads, cis_reads, trans_reads = normalize_matrix(matrix)
    if total_reads < 1:
        print("Input matrix %s is empty" % matrix)
    else:
        gini_idx = gini(normalized)
        adjusted_gini = adjust(gini_idx, total_reads)
        Raw_GiniQC = "%.3f" % gini_idx
        Adjusted_GiniQC = "%.3f" % adjusted_gini
        return Raw_GiniQC, Adjusted_GiniQC
