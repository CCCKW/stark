...
if __name__ == '__main__':
	...
	# 输入文件使用 rb 模式读取
	in_handles = np.array([
	    open_file(args.infile1, 'rb'),
	    open_file(args.infile2, 'rb')
	])

	# 输出文件使用 wb 模式写入
	out_handles = np.array([
	    open_file(args.outfile1, 'wb'),
	    open_file(args.outfile2, 'wb')
	])

	# 可选的 unknown 文件
	if args.unknown1 and args.unknown2:
	    out_handles_u = np.array([
	        open_file(args.unknown1, 'wb'),
	        open_file(args.unknown2, 'wb')
	    ])
	else:
	    out_handles_u = np.array([])
	if args.unknown1 and args.unknown2:
